...
for p in $(seq 1 112); do
  echo "Running p=$p..."
  # Важно: mpirun запускает python на p процессов
  # Отключаем openib на всякий случай
  line=$(mpirun -np $p --mca btl ^openib python3 metropolis_mpi.py $TOTAL_T $SIGMA $WRITE_CHAIN | tail -n 1)

  # ВАЖНО: сделай так, чтобы твой python-скрипт в конце печатал:
  # "NP=<p> TOTAL_T=<...> elapsed=<...>"
  # Тогда парсинг будет стабильный:
  elapsed=$(echo "$line" | sed -n 's/.*Elapsed time (Python + MPI): \([0-9.]*\).*/\1/p')

  # Если ты не меняла формат вывода — твоя строка сейчас: "Elapsed time (Python + MPI): X seconds"
  # sed выше вытаскивает X
